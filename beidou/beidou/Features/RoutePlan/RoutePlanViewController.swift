...
final class RoutePlanViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            stack.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 70),
    ...
    private func setupInputCard() -> UIView {
        ...
        swapButton.heightAnchor.constraint(equalToConstant: 30).isActive = true
        ...
        NSLayoutConstraint.activate([
            outerStack.topAnchor.constraint(equalTo: card.topAnchor, constant: 12),
            outerStack.bottomAnchor.constraint(equalTo: card.bottomAnchor, constant: -12),
            ...
            startRow.heightAnchor.constraint(equalToConstant: 36),
            endRow.heightAnchor.constraint(equalToConstant: 36)
        ])
        return card
    }
    ...
    private func setupModeButtons() -> UIView {
        let drive = makeModeButton(mode: .drive, icon: "car.fill", title: "驾车")
        let walk = makeModeButton(mode: .walk, icon: "figure.walk", title: "步行")
        let ride = makeModeButton(mode: .ride, icon: "bicycle", title: "骑行")
        let truck = makeModeButton(mode: .truck, icon: "shippingbox.fill", title: "货车")
    ...
    private func currentLocationPOI(name: String = "我的位置") -> SelectedPOI {
        if let location = startLocation {
            return SelectedPOI(name: name, address: "", latitude: location.latitude, longitude: location.longitude)
        }
        if let cached = LocationManager.shared.lastKnownLocation {
            return SelectedPOI(name: name, address: "", latitude: cached.latitude, longitude: cached.longitude)
        }
        return SelectedPOI(name: name, address: "", latitude: Constants.defaultStartLat, longitude: Constants.defaultStartLon)
    }
    ...
    @objc private func tapStartNavi() {
        ...
        if routeSwapped {
            ...
            end = currentLocationPOI(name: "原起点")
        } else {
